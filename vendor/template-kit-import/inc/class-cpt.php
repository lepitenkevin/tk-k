...
abstract class CPT extends Base {
	...
	public function get_cpt_args() {
		...
		return array(
			...
			/*
			 * These posts are an internal data store, not user content: they are created and deleted
			 * exclusively by our own code, which performs its own capability checks at the REST layer.
			 *
			 * They must therefore not be reachable through Core's generic post editing surfaces, so
			 * every capability is pinned to manage_options rather than inherited from "post". Our own
			 * flows are unaffected, as wp_insert_post() and wp_delete_post() do not check capabilities.
			 */
			'capabilities'        => array(
