...
abstract class Builder extends Base {
	...
	public function get_template_kit_temporary_folder() {
		...
		// Confirm we're pulling this data from the right location. The helper fully resolves the
		// path before checking containment, so callers can trust the result is inside our own
		// uploads folder, or null.
		return envato_template_kit_import_get_kit_folder_path( $template_kit_folder_name );
	}
	...
	public function get_template_kit_temporary_url() {
		...
		// Same stored value as the path above, so hold it to the same expected format.
		if ( ! envato_template_kit_import_is_valid_kit_folder_name( $template_kit_folder_name ) ) {
			return '';
		}

		$wp_upload_dir = wp_upload_dir();

		return trailingslashit( $wp_upload_dir['baseurl'] ) . 'template-kits/' . $template_kit_folder_name . '/';
	}
